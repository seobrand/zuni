...
function numbers_modified($options = array()) {
		...
		if ($modulus && $params['pageCount'] > $modulus) {
			...
			for ($i = $start; $i < $params['page']; $i++) {
				$out .= '<li>'.$this->Html->tag($tag, $this->link($i, array('page' => $i), $options))
					. $separator.'</li>';
			}

			$out .= $this->Html->tag($tag, $params['page'], array('class' => 'select'));
			if ($i != $params['pageCount']) {
				$out .= $separator;
			}
			...
			for ($i = 1; $i <= $params['pageCount']; $i++) {
				if ($i == $params['page']) {
					if($i==1) {
						$out .= '<li><span class="select"><a href="javascript:void(0)" style="font: 12px Arial, Helvetica, sans-serif;color: white;text-decoration:none;
background: black;padding: 1px 4px 1px 4px;">1</a></span></li>';
					} else {
					$out .= '<li><span class="select"><a href="javascript:void(0)">'.$i.'</a></span></li>';
					}
				} else {
					$out .= '<li>'.$this->Html->tag($tag, $this->link($i, array('page' => $i), $options)).'</li>';
				}
				if ($i != $params['pageCount']) {
					$out .= $separator;
				}
