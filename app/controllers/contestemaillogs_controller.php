<?php
class ContestemaillogsController extends AppController {
	  var $name = 'Contestemaillogs';
...
//------------------------------------------------------------------------------------------------------------------------//
	   function index(){
	   				$this->Contestemaillog->query("DELETE FROM contestemaillogs WHERE sending_date is null");
					if(isset($this->params['pass'][0])) {
						$lastlog = $this->Contestemaillog->find('first',array('conditions'=>array('Contestemaillog.unique'=>$this->params['pass'][0],'Contestemaillog.sending_date!=""'),'order'=>'Contestemaillog.id DESC'));
					} else {
						$lastlog = $this->Contestemaillog->find('first',array('conditions'=>array('Contestemaillog.sending_date!=""'),'order'=>'Contestemaillog.id DESC'));
					}
					...
					if(!empty($lastlog)) {
						$ids = $lastlog['Contestemaillog']['sent'].','.$lastlog['Contestemaillog']['notsent'];
						$idarr = implode(',',array_values(array_unique(array_filter(explode(',',$ids)))));
						$cond[] = "DiscountNewsletter.id IN (".$idarr.")";
						
						$delivered = implode(',',array_values(array_unique(array_filter(explode(',',$lastlog['Contestemaillog']['sent'])))));
						$cond1 = '';
						if($delivered) {
						...
						$opened = implode(',',array_values(array_unique(array_filter(explode(',',$lastlog['Contestemaillog']['email_opened'])))));
						$cond1 = '';
						if($opened) {
						...
						$link = implode(',',array_values(array_unique(array_filter(explode(',',$lastlog['Contestemaillog']['opened'])))));
						$cond1 = '';
						if($link) {
...
//------------------------------------------------------------------------------------------------------------------------//	
	function saveEmailOpen() {
		...
		if(count($breakstring)==2) {
			$this->common->saveContestEmailOpenStatus($breakstring[0],base64_decode($breakstring[1]));
		}
	}
//------------------------------------------------------------------------------------------------------------------------//
	/*
