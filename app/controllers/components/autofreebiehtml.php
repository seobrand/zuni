...
class autofreebiehtmlComponent extends Object {
	...
	function email_content($advertiser,$tracking_string='',$freebie='') {
		...
		$header = '';
			...
			if(isset($offers[0]['DailyDeal']['unique'])) {
				$advertiser_logo = 'https://zuni.com/img/deals/'.$offers[0]['DailyDeal']['banner_image'];
				$companyUrl = 'https://zuni.com/state/'.$state.'/'.$county.'/dailydeal?unique='.$offers[0]['DailyDeal']['unique'].$tracking_string;
			} else {
				$advertiser_logo = 'https://zuni.com/img/offer/soffers/'.$this->common->getAdvertiserOfferImagebyId($value);
				$companyUrl = 'https://zuni.com/state/'.$state.'/'.$county;
			}
