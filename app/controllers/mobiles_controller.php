...
class MobilesController extends AppController {
	var $name = 'Mobile'; //Model name attached with this controller
...
/********************** This function call for all county pages on front end ************************/
	function home($state='',$county='') {
		...
		//validate url on each hit on home action
		//auto popup for advertiser registration process
...
if($this->checkurl->validUrl($this->params)){
		...
		if($this->Session->read('Auth.FrontUser') && isset($this->params['pass'][2]) && $this->params['pass'][2]=='profile') {
			...
			$this->render('/mobile_users/discount_reedem');
		}
			...
			$this->layout = 'mobile_profile';
			$this->render('/mobile_users/discount');
		}
		else if(!$this->Session->read('Auth.FrontUser') && isset($this->params['pass'][2]) && ($this->params['pass'][2]=='profile' || $this->params['pass'][2]=='account' || $this->params['pass'][2]=='discount' || $this->params['pass'][2]=='feedback' || $this->params['pass'][2]=='discount_history' || $this->params['pass'][2]=='discount_reedem')) {
		...
		// 											     This section call Profile Pages of Consumer							        //
   	   //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		else if($this->Session->read('Auth.FrontConsumer') && isset($this->params['pass'][2]) && $this->params['pass'][2]=='order_history') {
			$this->layout = 'mobile_profile';
			$this->render('/mobile_users/order_history');
		}
		else if($this->Session->read('Auth.FrontConsumer') && isset($this->params['pass'][2]) && $this->params['pass'][2]=='bucks') {
			$this->layout = 'mobile_profile';
			$this->render('/mobile_users/bucks');
		}
		else if($this->Session->read('Auth.FrontConsumer') && isset($this->params['pass'][2]) && $this->params['pass'][2]=='change_category') {
			$this->layout = 'mobile_profile';
			...
			$this->render('/mobile_users/spend');
		}
			...
			if(!empty($detail)) {
					...
					$this->render('/mobile_users/spendBucks');
				} else {
					$this->redirect(FULL_BASE_URL.router::url('/',false).'state/'.$this->params['pass'][0].'/'.$this->params['pass'][1]);
				}
			} else {
			...
			$this->layout = 'mobile_profile';
			$this->render('/mobile_users/refer_business');
		}
		else if($this->Session->read('Auth.FrontConsumer') && isset($this->params['pass'][2]) && $this->params['pass'][2]=='refer_friend') {
			
						$this->layout = 'mobile_profile';
						$this->render('/mobile_users/refer_friend');
		}	   
		else if(!$this->Session->read('Auth.FrontConsumer') && isset($this->params['pass'][2]) && ($this->params['pass'][2]=='order_history' || $this->params['pass'][2]=='bucks' || $this->params['pass'][2]=='info' || $this->params['pass'][2]=='spend' || $this->params['pass'][2]=='spendBucks' || $this->params['pass'][2]=='refer_business' || $this->params['pass'][2]=='refer_friend')) {
			$this->redirect(FULL_BASE_URL.router::url('/',false).'state/'.$this->params['pass'][0].'/'.$this->params['pass'][1]);
		}
		  // Set default layout
		 //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	  ...
	  // $this->Cookie->write('LastUrl', $this->params['url']['url'], true, '+30 day');
			...
			// Set page type
			$this->Session->write('page_type','home');
			// set cats into session for daily discounts and deals
			$this->Session->write('discount_proof_cats','');
			//$this->Session->write('discount_proof_subcats','');			
				//manage the highlighted business area for home page
					 App::import('model','AdvertiserProfile');
			         $this->AdvertiserProfile=new AdvertiserProfile();					 
					 //$adv_list=$this->AdvertiserProfile->query("select `id`,`city`,`category`,`subcategory`,`page_url` from advertiser_profiles where `publish`='yes' and `show_at_home`=1 and `county`=".$this->Session->read('county_data.id')." ORDER BY RAND()");
					 $adv_list = $this->AdvertiserProfile->find('all',array('conditions'=>array('AdvertiserProfile.publish'=>'yes','AdvertiserProfile.county'=>$this->Session->read('county_data.id'))));
				//get current time stamp
					$cur_time=mktime(0,0,0,date('m'),date('d'),date('Y'));
				//fetch the saving offer
					foreach($adv_list as $adv_list1)
					{
						$s_offer_demo=$this->common->getmainSavingOfferImg_front($adv_list1['AdvertiserProfile']['id']);
						if(isset($s_offer_demo) and !empty($s_offer_demo))
						{
							$s_offer[]=$s_offer_demo;
							$adv_id[]=$adv_list1['AdvertiserProfile']['id'];
						}
					}
					if(!empty($s_offer)) {
						$this->set('high_business',$s_offer);
					}
					// new saving offer listing which is recently added (latest 4 offers)
					$this->set('newListedOffers',$this->common->getmainSavingOfferLatestAdded($this->params['pass'][1]));
	...
	/*--------------------------category specific search on hot button clicked--------------------*/				
		 //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		// 											     This section calls static page details								            //
   	   //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		elseif(0 && count($this->params['pass'])==4 && $this->params['pass'][2]=='pages' && ($this->checkurl->chkStaticPageUrl($this->params['pass'][3])!=0))
		{
		
			$this->layout = 'mobile_staticpage';
		...
		//find daily discount for today
		 ...
		 //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//												This section calls REFER A FRIEND												//
   	   //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////					
		elseif(count($this->params['pass'])<=3 && $this->params['pass'][2]=='refer_friend') {					
					$this->render('refer_friend');
		}
		...
		/*else if(!$this->Session->read('Auth.FrontConsumer') && isset($this->params['pass'][2]) && $this->params['pass'][2]=='login'){
		 ...
		 //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		// 											     This section calls Archive DAILY DISCOUNT							            //		
   	   //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////							
		elseif(count($this->params['pass'])<=3 && $this->params['pass'][2]=='archivediscount') {
					$this->set('title_for_layout','Archive Daily Discount : '.ucfirst($this->params['pass'][1]));
					$this->set('keyword_for_layout','Archive Daily Discount : '.ucfirst($this->params['pass'][1]));
					$this->set('description_for_layout','Archive Daily Discount : '.ucfirst($this->params['pass'][1]));
					$this->layout = 'mobile_archivediscount';
								//find daily discount for today
						$this->loadModel('DailyDiscount');
						$today = mktime(date('H'),date('i'),date('s'),date('m'),date('d'),date('Y'));
						//Find remaining daily discounts
						$all_daily_disc = $this->DailyDiscount->find('all',array('conditions'=>array("((DailyDiscount.e_date<$today AND DailyDiscount.show_on_home_page=1) OR (DailyDiscount.c_e_date<$today AND DailyDiscount.show_on_category=1)) AND DailyDiscount.status='yes' AND DailyDiscount.advertiser_county_id=".$this->Session->read('county_data.id'))));
						$this->set('all_daily_disc',$all_daily_disc);				
						$this->render('archivediscount');
				}
		 //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//---------*************************** commented during launch process for display same deal n discount as home page ******************-------------------//							
				...
				//find daily discount for today
					...
					//to find the advertiser profile data		
							 $map_address=$this->AdvertiserProfile->find('all',array('conditions'=>$conditions));
...
//---------------------------------------------------------------------------------------------------------------------------------//	
	function contact() {
        ...
        function staticpage($url){
           ...
           if(!$this->Session->read('state') || !$this->Session->read('county')){
		   		$this->Session->write('login_referer','pages/page/'.$url);
