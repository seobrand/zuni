...
//------------------------------------------------------------------------------------------------------------------------//
	   function index(){
	   				$this->Emaillog->query("DELETE FROM emaillogs WHERE sending_date is null");
					if(isset($this->params['pass'][0])) {
						$lastlog = $this->Emaillog->find('first',array('conditions'=>array('Emaillog.unique'=>$this->params['pass'][0],'Emaillog.sending_date!=""'),'order'=>'Emaillog.id DESC'));
					} else {
						$lastlog = $this->Emaillog->find('first',array('conditions'=>array('Emaillog.sending_date!=""'),'order'=>'Emaillog.id DESC'));
					}
					...
					if(!empty($lastlog)) {
						$ids = $lastlog['Emaillog']['sent'].','.$lastlog['Emaillog']['notsent'];
						$idarr = implode(',',array_values(array_unique(array_filter(explode(',',$ids)))));
						$cond[] = "DiscountNewsletter.id IN (".$idarr.")";
						
						$delivered = implode(',',array_values(array_unique(array_filter(explode(',',$lastlog['Emaillog']['sent'])))));
						$cond1 = '';
						if($delivered) {
						...
						$opened = implode(',',array_values(array_unique(array_filter(explode(',',$lastlog['Emaillog']['email_opened'])))));
						$cond1 = '';
						if($opened) {
						...
						$link = implode(',',array_values(array_unique(array_filter(explode(',',$lastlog['Emaillog']['opened'])))));
						$cond1 = '';
						if($link) {
	    			...
	    			$this->set('title_for_layout', 'Big Deal email tracking log');
...
//------------------------------------------------------------------------------------------------------------------------//	
	function saveEmailOpen() {
		...
		if(count($breakstring)==2) {
			$this->common->saveEmailOpenStatus($breakstring[0],base64_decode($breakstring[1]));
		}
	}
//------------------------------------------------------------------------------------------------------------------------//
	/*
