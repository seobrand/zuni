...
$contents="<script type='text/javascript'>
	...
	function old_countNumberContacts(currentCheckbox){
		var cnt = 0;
		for(z=1; z<=1000;z++){
		...
		if(cnt == 2){
			...
			alert('You can import only one contact at a time.');
		}
		...
		if ($inviter->showContacts())
		   ...
		   echo '<script language="javascript">self.resizeTo(900, 600);</script>';
			...
			if (count($contacts)==0)
				...
				{
				$contents.="<table class='thTable' align='center' cellspacing='0' cellpadding='0' width='650'><tr class='thTableHeader'><td colspan='".($plugType=='email'? "3":"2")."'>Your contacts</td></tr><tr class='thTableDesc'><td></td><td>Add</td><td>Name</td>".($plugType == 'email' ?"<td>E-mail</td>":"")."</tr>";
				$odd=true;$counter=0;
...
for (var j=0;j<=checkboxCount; j++) {
     if(document.getElementById('check_'+j)!=null && document.getElementById('check_'+j).checked==true) {	
	 	window.opener.document.getElementById('referName').value = document.getElementById('first_name_'+j).value+' '+document.getElementById('last_name_'+j).value;
		window.opener.document.getElementById('referEmail').value = document.getElementById('email_'+j).value;
     }
}
	window.close();
}
